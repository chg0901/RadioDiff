            ...
            while self.step < self.train_num_steps:
                ...
                for ga_ind in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        ...
                        # Check for NaN in log_dict values
                        ...
                        if torch.isnan(torch.tensor(loss_simple)):
                            loss_simple = 0.0
                        if torch.isnan(torch.tensor(loss_vlb)):
                            loss_vlb = 0.0
                ...
                # Use adaptive gradient clipping with increased threshold
                grad_norm = torch.nn.utils.clip_grad_norm_(
                    filter(lambda p: p.requires_grad, self.model.parameters()), 
                    max_norm=1.0,  # Increased from 0.5 for better stability
                    norm_type=2.0
                )
